...
STATIC mali_error gpu_trace_init(struct kbase_device *kbdev)
{
...
static int gpu_context_init(struct kbase_device *kbdev)
{
	...
	kbdev->vendor_callbacks = (struct kbase_vendor_data *)kbasep_get_config_value(kbdev, kbdev->config_attributes, KBASE_CONFIG_ATTR_VENDOR_CALLBACKS);

#ifdef CONFIG_MALI_EXYNOS_TRACE
	if (gpu_trace_init(kbdev) != MALI_ERROR_NONE)
		return -1;
#endif
	return 0;
}
...
extern struct kbase_vendor_callbacks exynos_callbacks;
...
#endif
	{
		KBASE_CONFIG_ATTR_VENDOR_CALLBACKS,
		(uintptr_t)&exynos_callbacks
	},
	{
